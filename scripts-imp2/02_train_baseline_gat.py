"""
02_train_baseline_gat.py: Train baseline GAT model with comprehensive metrics.

This script:
- Loads preprocessed graph data
- Creates a baseline 2-layer GAT (64 hidden, 4 heads)
- Trains with CrossEntropyLoss + class weights
- Implements early stopping (patience=20)
- Tracks: loss curves, macro/micro F1, per-class metrics, confusion matrix, ROC-AUC
...
from src.config import ARTIFACTS_DIR, MODEL_CONFIG, TRAINING_CONFIG
...
print(f"✓ Features: {data.x.shape[1]}, Classes: {data.y.max().item() + 1}")
...
for epoch in range(1, epochs + 1):
    ...
    if val_f1_macro > best_val_f1:
        ...
        # Save best model
        best_model_path = ARTIFACTS_DIR / "baseline_gat_best.pt"
...
# Save JSON
metrics_path = ARTIFACTS_DIR / "baseline_gat_metrics.json"
...
plt.tight_layout()
plot_path = ARTIFACTS_DIR / "baseline_gat_training_curves.png"
